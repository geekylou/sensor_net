...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  ...
  systime_t end = start + MS2ST(50);

  do writeReg(REG_SYNCVALUE1, 0xAA); while (readReg(REG_SYNCVALUE1) != 0xaa && chVTIsSystemTimeWithin(start, end));
  
  start = chVTGetSystemTime();
  end = start + MS2ST(50);
  
  do writeReg(REG_SYNCVALUE1, 0x55); while (readReg(REG_SYNCVALUE1) != 0x55 && chVTIsSystemTimeWithin(start, end));
  ...
  // This is best done on Chibios Manually
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
  ...
  // write to FIFO
  select();
  uint8_t data[] = {REG_FIFO | 0x80, bufferSize + 3, toAddress, _address, CTLbyte};
  spiSend(_spi, 5, data);
  spiSend(_spi, bufferSize, buffer);
  ...
  systime_t start = chVTGetSystemTime();
  systime_t end = start + MS2ST(RF69_TX_LIMIT_MS);
  
  while (_interruptChannel->Get() == 0 && chVTIsSystemTimeWithin(start, end)); // wait for DIO0 to turn HIGH signalling transmission finish
...
void RFM69::interruptHandler() {
  ...
  if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
  {
    ...
    spiReceive(_spi, 1, (void *)&TARGETID);
